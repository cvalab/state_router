...
class HistoryChange {
  ...
  String toString() {
    ...
    //MaterialPageRoute newRoutej = newRoute as MaterialPageRoute;
    if (newRoute != null) {
      if (newRoute?.settings != null) {
        print('newRoute.settings=${newRoute?.settings}');
        if (newRoute!.settings is Page) {
          if (newRoute?.settings is MaterialPage ||
              newRoute?.settings is CupertinoPage) {
      ...
      //
      else if (newRoute?.settings is RouteSettings) {
        if (newRoute?.settings.name != null) {
          routeName = newRoute?.settings.name;
        } else {
          // routeName =(newRoute as MaterialPage) ;
        }
      }
    }
    ...
    return 'HistoryChange{action: $action, \nnewRoute: $newRoute, page=${page}, newRoute.settings=${newRoute?.settings} \noldRoute: $oldRoute}\n\n';
  }
}
